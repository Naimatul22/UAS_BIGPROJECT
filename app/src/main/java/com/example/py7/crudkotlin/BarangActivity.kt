...
class BarangActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            if (id !=0){
                txText.setText(bundle.getString("MainActText"))
                txSena.setText(bundle.getString("MainActSentiment"))
                txPro.setText(bundle.getString("MainActProbability"))
            }
        }catch (ex: Exception){
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            R.id.action_save -> {
                ...
                values.put("Text", txText.text.toString())
                values.put("Sentiment", txSena.text.toString())
                values.put("Probability", txPro.text.toString())
